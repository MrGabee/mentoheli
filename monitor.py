name: 🚁 Mentőhelikopter Monitor

on:
  workflow_dispatch:
  repository_dispatch:
    types: [mentoheli_trigger]
    ...
    steps:
      ...
      - name: 💾 Állapot commitolása
        run: |
          ...
          git stash pop || true
          [ -f allapot.json ] && git add allapot.json || echo "fájl nem létezik"
      ...
      - name: 🗑️ Régi futások törlése
        if: always()
        run: |
          gh run list --workflow=main.yml --limit 50 \
      ...
      - name: 🔄 Következő futás indítása
        ...
        run: |
          curl -s -X POST \
            ...
            -d '{"event_type":"mentoheli_trigger"}'
